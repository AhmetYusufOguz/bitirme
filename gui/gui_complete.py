"""
GUI ve Visualization modülü - Birleştirilmiş (FINAL VERSION)
...
# Metrics modülün 'metrics' klasörü altında 'metrics_all.py' içinde olduğu için:
...
class Visualizer:
    ...
    def plot_dashboard_map(problem, solution, title: str = "Optimized Relief Routes"):
        """Creates the 'Professional' Square Map View"""
    ...
    def generate_schedule_text(problem, solution):
        ...
        # --- DÜZELTME: Algoritma İsmi ve Çözüm Sayısı ---
        ...
        for i, route in enumerate(sorted_routes):
            ...
            # --- DÜZELTME: SAAT AYARI ---
            # problem.py'de mesai 08:00 (480. dakika) başlıyor.
            # Burayı 480 yaptık ki loglar 'LATE' veya 'WAIT' diye saçmalamasın.
            current_time = 480 
...
class DisasterReliefGUI: 
    def __init__(self):
        ...
        # --- KRİTİK DEĞİŞKEN ---
        # En iyi çözümü burada saklayacağız
    ...
    def _create_parameter_panel(self, parent):
        ...
        ttk.Button(parent, text="Load Fixed Test", command=self._load_fixed_test, style="Accent.TButton").grid(row=row, column=0, columnspan=2, pady=5, sticky="ew")
        row += 1
    ...
    def _run_algorithms_thread(self):
        """Akıllı Kıyaslama: En iyi çözümü bul, kaydet ve metrikleri bas"""
        self.results = {}
            ...
            try:
                ...
                if result.size() > 0:
                    ...
                    if is_new_winner:
                        ...
                        # Anlık olarak hafızaya at (güvenlik için)
                        self.current_best_solution = global_best_solution
        ...
        # --- DÜZELTME: Şampiyonu Kalıcı Hafızaya Kaydet ---
    ...
    def _get_best_solution(self):
        """
        DÜZELTME: Artık Thread içinde seçilen 'Şampiyonu' (current_best_solution) döndürür.
        Rastgele 0. elemanı seçip 'Unknown' deme hatası giderildi.
        """
        # 1. Eğer thread çalıştıysa ve şampiyonu kaydettiyse, direkt onu ver
        if self.current_best_solution is not None:
            # İsmini al (yoksa varsayılan ata)
            algo_name = getattr(self.current_best_solution, 'algo_name', 'Unknown Algorithm')
            return algo_name, self.current_best_solution

        # 2. Fallback (Eğer henüz optimizasyon yapılmadıysa)
        if self.results:
            first_algo = list(self.results.keys())[0]
            if self.results[first_algo].size() > 0:
                solutions = self.results[first_algo].get_solutions()
                best = min(solutions, key=lambda s: s.f2_operational_cost)
                if not hasattr(best, 'algo_name'):
                    best.algo_name = first_algo
                    best.pareto_size_found = self.results[first_algo].size()
                return first_algo, best
